var ic_config = {
		...
		//视图所在路径
		viewPath:"/index/main",
		//1:识别基础信息和证件照； 2：只识别基础信息；3：只识别证件照。
		sb_flag:1,
		//回调更新界面的url标志，1：更新人工登记；2、更新备案砝码；3、更新核辐射界面； 4、证书打印界面
		viewFlag:1,
		//是否开启二维码扫描1：开启，0：不开起
		qRSwitch:1
};
function startOCX(path, p_flag, viewType){
	...
	if(ic_config.stopLoop != -1){//如果有循环，则终止。
		stopOCX();
	}
	...
	if(ic_config.ocxFlag == 0){
		ic_config.stopLoop = setInterval("AutoClassifyCard2()", 1000);
		ic_config.ocxFlag = 1;
	}
}
function stopOCX(){
...
function picToBase64Code(){  
     if(objIDCard.IsLoaded()){
        var nResult=objIDCard.EncodeToBase64(ic_config.cardImgSrc);
        return nResult;
     }else{
    	 alert("证件识别机未加载。");
     }
...
function AutoClassifyCard(){
	if("undefined" != typeof objIDCard){
		if(objIDCard.IsLoaded()){
			...
			if( nRet == 1){ 
				var qrStr=1;
				if( objIDCard.AcquireImage(0)==0 ){
					if(ic_config.qRSwitch == 1){//判断是否扫描二维码
						qrStr=objIDCard.GetQRcode();
					}
					if(qrStr==1){//表示识别二维码失败，则去判断其他证件的识别。
						classifyOtherCard();
					}else{//识别二维码成功后，处理二维码数据。
						if(qrStr.length<26){
							$.alert("识别失败，请重新识别！");
						}
